...
const WelcomeForm: React.FC = () => {
  ...
  const handleOpenInvitation = () => {
    ...
    // Extract guestId and invitationId from the path if present
    ...
    if (pathParts.length > 0) {
      ...
      // Check if the URL contains a combined invitationId-guestId format
      if (segment.includes('-')) {
        const [invitationId, guestId] = segment.split('-');
        path = `/invitation/${invitationId}-${guestId}`;
      } else {
        // Just an invitationId
        path = `/invitation/${segment}`;
      }
    }
