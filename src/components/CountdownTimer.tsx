...
const CountdownTimer: React.FC = () => {
  ...
  return (
    <section id="countdown-timer" className="w-full py-2 mb-2">
      <div className="w-full max-w-3xl mx-auto px-4">
        <div className="text-center mb-2">
          ...
          <h3 className="font-dancing-script text-xl sm:text-2xl text-wedding-maroon">
            Counting Down To Our Special Day
          </h3>
        </div>
        
        <div 
          className={`glass-card py-2 sm:py-3 px-2 border border-wedding-gold/20 shadow-gold-soft hover:shadow-gold-glow transition-all duration-500 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} cursor-pointer`}
            ...
            {timeUnits.map((unit, index) => (
              ...
              >
                <div className="relative inline-flex flex-col">
                  <div className={`${isMobile ? 'w-14 h-14' : 'w-16 h-16 sm:w-18 sm:h-18'} rounded-lg bg-gradient-to-br from-wedding-blush to-wedding-cream flex items-center justify-center shadow-md relative overflow-hidden group`}>
                    <div className="absolute inset-0 bg-wedding-gold/5 group-hover:bg-wedding-gold/10 transition-colors duration-300"></div>
                    <span className={`font-playfair ${isMobile ? 'text-xl' : 'text-2xl'} text-wedding-maroon relative z-10`}>
                  ...
                  <p className={`mt-1 ${isMobile ? 'text-[10px]' : 'text-xs'} text-gray-600`}>{unit.label}</p>
                </div>
              </div>
            ))}
          </div>
